...
$query = "
    SELECT 
        ...
        (SELECT FORMAT(AVG(weight), 2) FROM child_nutrition) AS avg_weight,
        (SELECT FORMAT(AVG(height), 2) FROM child_nutrition) AS avg_height,
        (SELECT FORMAT(AVG(blood_pressure), 2) FROM child_nutrition) AS avg_blood_pressure,
        (SELECT FORMAT(AVG(heart_rate), 2) FROM child_nutrition) AS avg_heart_rate,
        (SELECT FORMAT(AVG(temperature), 2) FROM child_nutrition) AS avg_temperature;
";
