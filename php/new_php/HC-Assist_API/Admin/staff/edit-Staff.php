...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Prepare SQL query
    $sql = "UPDATE staff SET position=?, password=?, first_name=?, middle_name=?, last_name=?, age=?, birth_date=?, gender=?, civil_status=?, purok_assigned=?, contact_number=?, image=?
            WHERE staff_id=?";

    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("sssssissssssi", $position, $password, $first_name, $middle_name, $last_name, $age, $birth_date, $gender, $civil_status, $purok_assigned, $contact_number, $imagePath, $staff_id);
        if ($stmt->execute()) {
